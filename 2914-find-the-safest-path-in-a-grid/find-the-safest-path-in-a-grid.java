    ...
    public int maximumSafenessFactor(List<List<Integer>> grid) {
        ...
        while(!multiSourceQueue.isEmpty()) {
            ...
            while(size -- > 0) {
                ...
                for(int d[] : dir) {
                    int diri = curr[0] + d[0], dirj = curr[1] + d[1], val = mat[curr[0]][curr[1]];
                    if(isValid(mat, diri, dirj) && mat[diri][dirj] == -1) {
                        mat[diri][dirj] = val + 1;
                        multiSourceQueue.add(new int[]{diri, dirj});
                    }
                }
            }
        }
        ...
        while(!pq.isEmpty()) {
            ...
            for (int[] d : dir) {
                int diri = d[0] + curr[0], dirj = d[1] + curr[1];
                if (isValid(mat, diri, dirj) && mat[diri][dirj] != -1) {
                    pq.add(new int[] {diri, dirj, Math.min(curr[2], mat[diri][dirj])});
                    mat[diri][dirj] = -1;
                }
            }
        }
